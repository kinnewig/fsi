...
template <int dim, typename Number, typename MemorySpace>
void
OptimizedFROSchPreconditioner<dim, Number, MemorySpace>::distribute_matrix (
  ...
  // turn the vector into a xpetra vector:
  Teuchos::RCP<LA::XpetraTypes::CrsMatrixType<Number, MemorySpace>> crs_global_matrix =
    Teuchos::rcp(new LA::XpetraTypes::TpetraCrsMatrixType<Number, MemorySpace>(global_matrix.trilinos_rcp()));
  Teuchos::RCP<LA::XpetraTypes::MatrixType<Number, MemorySpace>> xpetra_global_matrix =
    Teuchos::rcp(new LA::XpetraTypes::CrsMatrixWrapType<Number, MemorySpace>(crs_global_matrix));

  // Create the communcation plan, and create the global vector onto the overlapping one
  Teuchos::RCP<Xpetra::Import<LA::LO, LA::GO, LA::NO<MemorySpace> > > scatter = 
    Xpetra::ImportFactory<LA::LO, LA::GO, LA::NO<MemorySpace> >::Build(
      xpetra_global_matrix->getRowMap(), overlapping_map);
  Teuchos::RCP<LA::XpetraTypes::MatrixType<Number, MemorySpace> > xpetra_subdomain_matrix = 
    Xpetra::MatrixFactory<Number, LA::LO, LA::GO, LA::NO<MemorySpace>>::Build(overlapping_map, xpetra_global_matrix->getGlobalMaxNumRowEntries());
  // Communicate the solution vector onto the subdomain vector:
  xpetra_subdomain_matrix->doImport(*xpetra_global_matrix, *scatter, Xpetra::ADD);
  ...
  Teuchos::RCP<LA::XpetraTypes::MatrixType<Number, MemorySpace> > xpetra_local_subdomain_matrix = 
    Xpetra::MatrixFactory<Number, LA::LO, LA::GO, LA::NO<MemorySpace>>::Build(local_subdomain_map, xpetra_global_matrix->getGlobalMaxNumRowEntries());

  for (unsigned i=0; i< local_subdomain_map->getLocalNumElements(); i++) 
    {
      Teuchos::ArrayView<const LA::GO> indices;
      Teuchos::ArrayView<const Number> values;
      xpetra_subdomain_matrix->getGlobalRowView(overlapping_map->getGlobalElement(i), indices, values);

      LA::LO size = indices.size();
      if (size > 0) 
        {
          Teuchos::Array<LA::GO> indicesLocal;
          Teuchos::Array<Number> valuesLocal;
          for (LA::LO j = 0; j<size; j++)   
            {
              LA::GO localIndex = overlapping_map->getLocalElement(indices[j]);
              if (localIndex >= 0) 
                {
          ...
          xpetra_local_subdomain_matrix->insertGlobalValues(i, indicesLocal(), valuesLocal());
        }
    }


  Teuchos::RCP<LA::TpetraTypes::MatrixType<Number, MemorySpace>> tpetra_local_subdomain_matrix =
    Xpetra::toTpetra(xpetra_local_subdomain_matrix); 
  local_subdomain_matrix.trilinos_rcp() = tpetra_local_subdomain_matrix;
}
