...
namespace FSI
{
  ...
  namespace NSE_in_ALE
  {
    ...
    template <int dim>
    inline Tensor<2, dim>
    get_stress_fluid_ALE_1st_term_LinAll(
      const Tensor<2, dim> pI,
      const Tensor<2, dim> F_Inverse_T,
  ...
  namespace Parameters
  {
    ...
    GlobalValues::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Global");
      {
        ...
        prm.declare_entry("no_of_refinements",
                          "1",
                          Patterns::Integer(0),
                          "no_of_refinements");
      }
    ...
    PhysicalConstants::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Physical constants");
      {
        ...
        prm.declare_entry("force_structure_x",
                          "0.0",
                          Patterns::Double(0),
                          "fx");
        prm.declare_entry("force_structure_y",
                          "0.0",
                          Patterns::Double(0),
                          "fy");
    ...
    Time::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection(
        "Timestepping schemes & timestep & max_no_timesteps");
      {
        ...
        prm.declare_entry("max_no_timesteps",
                          "25",
                          Patterns::Integer(0),
                          "number of timesteps");
      }
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::set_runtime_parameters()
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::setup_system()
  {
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::assemble_system_matrix()
  {
    ...
    std::vector<Vector<double>> old_solution_face_values(
    ...
    std::vector<Vector<double>> old_timestep_solution_values(
    ...
    std::vector<Vector<double>> old_timestep_solution_face_values(
      n_face_q_points, Vector<double>(dim + dim + 1));


    std::vector<std::vector<Tensor<1, dim>>> old_timestep_solution_face_grads(
      n_face_q_points, std::vector<Tensor<1, dim>>(dim + dim + 1));
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                // the ALE mapping of the fluid equations. Second, these terms
                // are used to make the notation as simple and self-explaining
                // as possible:
                const Tensor<2, dim> pI =
                ...
                const Tensor<1, dim> u =
                  ALE_Transformations ::get_u<dim>(q, old_solution_values);
                ...
                // Further, we also need some information from the previous time
                // steps
                const Tensor<1, dim> old_timestep_v =
                  ALE_Transformations ::get_v<dim>(
                    q, old_timestep_solution_values);

                const Tensor<1, dim> old_timestep_u =
                  ALE_Transformations ::get_u<dim>(
                    q, old_timestep_solution_values);

                const Tensor<2, dim> old_timestep_F =
                  ALE_Transformations ::get_F<dim>(q,
                                                   old_timestep_solution_grads);

                const double old_timestep_J =
                  ALE_Transformations ::get_J<dim>(old_timestep_F);
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  {
                    ...
                    const Tensor<1, dim> accelaration_term_LinAll =
                      NSE_in_ALE ::get_accelaration_term_LinAll(phi_i_v[i],
                                                                v,
                                                                old_timestep_v,
                                                                J_LinU,
                                                                J,
                    ...
                    const Tensor<1, dim> convection_fluid_u_LinAll_short =
                      NSE_in_ALE ::get_Convection_u_LinAll_short<dim>(
                        phi_i_grads_v[i],
                        phi_i_u[i],
                        J,
                        J_LinU,
                    ...
                    const Tensor<1, dim> convection_fluid_u_old_LinAll_short =
                      NSE_in_ALE ::get_Convection_u_old_LinAll_short<dim>(
                        phi_i_grads_v[i],
                        J,
                        J_LinU,
                        F_Inverse,
                    ...
                    for (unsigned int j = 0; j < dofs_per_cell; ++j)
                      {
                        ...
                        if (comp_j == 0 || comp_j == 1)
                          {
                            local_matrix(j, i) +=
                              (accelaration_term_LinAll * phi_i_v[j] +
                               timestep * theta *
                                 convection_fluid_LinAll_short * phi_i_v[j] -
                               convection_fluid_u_LinAll_short * phi_i_v[j] +
                               convection_fluid_u_old_LinAll_short *
                                 phi_i_v[j] +
                               timestep * scalar_product(
                                            stress_fluid_ALE_1st_term_LinAll,
                                            phi_i_grads_v[j]) +
                               timestep * theta *
                                 scalar_product(
                        ...
                        else if (comp_j == 2 || comp_j == 3)
                          {
                            ...
                            local_matrix(j, i) +=
                              (-alpha_u / (J * J) * J_LinU *
                                 scalar_product(grad_u, phi_i_grads_u[j]) +
                               alpha_u / J *
                                 scalar_product(phi_i_grads_u[i],
                                                phi_i_grads_u[j])) *
                              fe_values.JxW(q);
                          }
                        else if (comp_j == 4)
                          {
                            local_matrix(j, i) +=
                              (incompressibility_ALE_LinAll * phi_i_p[j] +
                               alpha_press_stab * cell_diameter *
                                 cell_diameter * phi_i_grads_p[i] *
                                 phi_i_grads_p[j]) *
                              fe_values.JxW(q);
                          }
                        // end j dofs
                ...
                if (cell->face(face)->at_boundary() &&
                    (cell->face(face)->boundary_id() == 1))
                  {
                    ...
                    fe_face_values.get_function_values(
                      solution, old_solution_face_values);
                    fe_face_values.get_function_gradients(
                      solution, old_solution_face_grads);
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                const Tensor<2, dim> F_T =
                  ALE_Transformations ::get_F_T<dim>(F);
                    ...
                    for (unsigned int j = 0; j < dofs_per_cell; ++j)
                      {
                        ...
                        if (comp_j == 0 || comp_j == 1)
                          {
                            local_matrix(j, i) +=
                              (density_structure * phi_i_v[i] * phi_i_v[j] +
                               timestep * theta *
                                 scalar_product(
                                   piola_kirchhoff_stress_structure_STVK_LinALL,
                                   phi_i_grads_v[j])) *
                          ...
                          {
                            // Pressue with DGP
                            // local_matrix(j,i) += (phi_i_p[i] * phi_i_p[j]) *
                            // fe_values.JxW(q);
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                const Tensor<2, dim> old_timestep_grad_v =
                  ALE_Transformations ::get_grad_v<dim>(
                    q, old_timestep_solution_grads);

                const Tensor<2, dim> old_timestep_grad_v_T =
                  ALE_Transformations ::get_grad_v_T<dim>(old_timestep_grad_v);
                ...
                const Tensor<2, dim> old_timestep_F_Inverse =
                  ALE_Transformations ::get_F_Inverse<dim>(old_timestep_F);

                const Tensor<2, dim> old_timestep_F_Inverse_T =
                ...
                const Tensor<2, dim> old_timestep_sigma_ALE =
                  NSE_in_ALE ::get_stress_fluid_except_pressure_ALE<dim>(
                    density_fluid,
                    viscosity,
                    old_timestep_grad_v,
                    old_timestep_grad_v_T,
                    old_timestep_F_Inverse,
                ...
                old_timestep_stress_fluid =
                  (old_timestep_J * old_timestep_sigma_ALE *
                   old_timestep_F_Inverse_T);

                // Divergence of the fluid in the ALE formulation
                const double incompressiblity_fluid =
                  NSE_in_ALE ::get_Incompressibility_ALE<dim>(
                    q, old_solution_grads);
                ...
                // The second convection term for the fluid in the ALE
                // formulation
                ...
                // The third convection term for the fluid in the ALE
                // formulation
                ...
                old_timestep_convection_fluid =
                  (density_fluid * old_timestep_J *
                   (old_timestep_grad_v * old_timestep_F_Inverse *
                    old_timestep_v));
                    ...
                    if (comp_i == 0 || comp_i == 1)
                      {
                        ...
                        local_rhs(i) -=
                          (density_fluid * (J + old_timestep_J) / 2.0 *
                           ...
                           timestep * (1.0 - theta) *
                             scalar_product(old_timestep_stress_fluid,
                                            phi_i_grads_v)) *
                          fe_values.JxW(q);
                      }
                    ...
                    else if (comp_i == 4)
                      {
                        ...
                        local_rhs(i) -=
                          (incompressiblity_fluid * phi_i_p +
                           alpha_press_stab * cell_diameter * cell_diameter *
                             grad_p * phi_i_grads_p) *
                          fe_values.JxW(q);
                      }
                    // end i dofs
                  }
                 ...
                 ++face)
              {
                if (cell->face(face)->at_boundary() &&
                    (cell->face(face)->boundary_id() == 1))
                  {
                    ...
                    fe_face_values.get_function_values(
                      old_timestep_solution, old_timestep_solution_face_values);
                    fe_face_values.get_function_gradients(
                      old_timestep_solution, old_timestep_solution_face_grads);
                    ...
                    for (unsigned int q = 0; q < n_face_q_points; ++q)
                      {
                        ...
                        const Tensor<2, dim> old_timestep_grad_v =
                          ALE_Transformations ::get_grad_v<dim>(
                            q, old_timestep_solution_face_grads);

                        const Tensor<2, dim> old_timestep_grad_v_T =
                          ALE_Transformations ::get_grad_v_T<dim>(
                            old_timestep_grad_v);
                        ...
                        const Tensor<2, dim> old_timestep_F_Inverse =
                          ALE_Transformations ::get_F_Inverse<dim>(
                            old_timestep_F);
                        ...
                        old_timestep_sigma_ALE_tilde =
                          (density_fluid * viscosity *
                           old_timestep_F_Inverse_T * old_timestep_grad_v_T);
                        ...
                        old_timestep_stress_fluid_transposed_part =
                          (old_timestep_J * old_timestep_sigma_ALE_tilde *
                           old_timestep_F_Inverse_T);
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                const Tensor<2, dim> old_timestep_F_T =
                  ALE_Transformations ::get_F_T<dim>(old_timestep_F);
                ...
                const Tensor<2, dim> old_timestep_E =
                  Structure_Terms_in_ALE ::get_E<dim>(old_timestep_F_T,
                                                      old_timestep_F,
                                                      Identity);

                const double old_timestep_tr_E =
                  Structure_Terms_in_ALE ::get_tr_E<dim>(old_timestep_E);
                ...
                old_timestep_sigma_structure_ALE =
                  (1.0 / old_timestep_J * old_timestep_F *
                   (lame_coefficient_lambda * old_timestep_tr_E * Identity +
                    2 * lame_coefficient_mu * old_timestep_E) *
                   old_timestep_F_T);

                Tensor<2, dim> old_timestep_stress_term;
                old_timestep_stress_term.clear();
                old_timestep_stress_term =
                  (old_timestep_J * old_timestep_sigma_structure_ALE *
                   old_timestep_F_Inverse_T);
                ...
                old_timestep_structure_force[0] =
                  density_structure * force_structure_x;
                old_timestep_structure_force[1] =
                  density_structure * force_structure_y;
                    ...
                    if (comp_i == 0 || comp_i == 1)
                      {
                        ...
                        local_rhs(i) -=
                          (density_structure * (v - old_timestep_v) * phi_i_v +
                           timestep * theta *
                             scalar_product(stress_term, phi_i_grads_v) +
                           timestep * (1.0 - theta) *
                             scalar_product(old_timestep_stress_term,
                                            phi_i_grads_v) -
                           timestep * theta * structure_force * phi_i_v -
                           timestep * (1.0 - theta) *
                             old_timestep_structure_force * phi_i_v) *
                          fe_values.JxW(q);
                      }
                    else if (comp_i == 2 || comp_i == 3)
                      {
                        ...
                        local_rhs(i) -=
                          (density_structure *
                           ((u - old_timestep_u) * phi_i_u -
                            timestep *
                              (theta * v + (1.0 - theta) * old_timestep_v) *
                              phi_i_u)) *
                          fe_values.JxW(q);
                      }
                    else if (comp_i == 4)
                      {
                        ...
                        // Pressure with DGP
                        // local_rhs(i) -= (old_solution_values[q](dim+dim) *
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::compute_drag_lift_fsi_fluid_tensor()
  {
                ...
                if (cell->material_id() !=
                    ...
                    cell->face(face)->boundary_id() != 80)
                  {
                    ...
                    for (unsigned int q_point = 0; q_point < n_face_q_points;
                      ...
                      {
                        const Tensor<2, dim> pI =
                          ALE_Transformations ::get_pI<dim>(
                        ...
                        drag_lift_value -=
                          1.0 * (stress_fluid + fluid_pressure) *
                          fe_face_values.normal_vector(q_point) *
                          fe_face_values.JxW(q_point);
                      }
                  }
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::compute_drag_lift_fsi_fluid_tensor_domain()
  {
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::compute_drag_lift_fsi_fluid_tensor_domain_structure()
  {
    unsigned int drag_lift_select   = 0;
    double       drag_lift_constant = 1.0;
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::compute_minimal_J()
  {
    QGauss<dim>        quadrature_formula(parameters.degree + 2);
    FEValues<dim>      fe_values(fe,
                            quadrature_formula,
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::compute_functional_values()
  {
    double x1, y1;
  ...
  template <int dim>
  void
  FSI_ALE_Problem<dim>::refine_mesh()
  {
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        // Refine the solid
